package sorterchain

import (
	"github.com/DmitryVasilkovW/Git-repository-analyzer.git/internal/model"
	"github.com/DmitryVasilkovW/Git-repository-analyzer.git/internal/service/sorter"
)

type SortByLinesHandler struct {
	BaseHandler
}

func (h *SortByLinesHandler) Handle(request model.RepoFlags, result []model.UserData) bool {
	if request.OrderBy == "lines" {
		sorter.SortByLine(result)
		return true
	}
	return h.BaseHandler.Handle(request, result)
}
